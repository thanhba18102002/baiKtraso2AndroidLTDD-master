...
//    }
    ...
    private void deleteRecord(String id){
        DatabaseReference Dbref = FirebaseDatabase.getInstance().getReference("sanphams");
        Task<Void> mTask = Dbref.removeValue();
        mTask.addOnSuccessListener(new OnSuccessListener<Void>() {
            @Override
            public void onSuccess(Void unused) {
                Intent intent=new Intent(CapNhatContactActivity.this, MainActivity.class);
        ...
        myRef.child(key).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                try {
                    ...
                    sanPham.setContactId(dataSnapshot.getKey());
                    edtId.setText(sanPham.getContactId());
                    edtTen.setText(sanPham.getTensp());
                    edtPhone.setText(sanPham.getGiasp());
                    edtEmail.setText(sanPham.getLoaisp());
...
//Kết nối tới node có tên là sanphams (node này do ta định nghĩa trong CSDL Firebase)
            DatabaseReference myRef = database.getReference("sanphams");
